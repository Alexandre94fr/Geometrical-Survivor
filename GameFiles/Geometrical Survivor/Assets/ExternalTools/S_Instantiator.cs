...
public class S_Instantiator : MonoBehaviour
{
    ...
    /// {
    ///     public static CLASS_NAME _Instance;
    /// 
    ///     public void Awake()
    ///     {
    ///         Instance = S_Instantiator.ReturnInstance(this, S_Instantiator.p_instanceConflictResolution.WarningAndPause);
    ///     }
    /// }
    ...
    public static T ReturnInstance<T>(T p_classInstance, InstanceConflictResolutions p_instanceConflictResolution) where T : MonoBehaviour
    {
        ...
        FieldInfo instanceVariable = givenClassType.GetField("_Instance", BindingFlags.Public | BindingFlags.Static);

        // If we didn't find the so called named "Instance" variable we return an error
        if (instanceVariable == null)
        {
            Debug.LogError($"ERROR ! The class {givenClassType} does not have a public static '_Instance' variable.");
            return null;
        }
